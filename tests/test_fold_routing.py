...
def test_an_arm_generated_by_the_wrong_adapter_is_caught():
    ...
    try:
        ...
        assert "lora_r16" in str(error) and "lora_r8" in str(error)
        return
...
def test_routing_survives_the_environment_boundary():
    """Generation runs on Colab and this audit runs on the laptop, so the
    recorded adapter is an absolute /content/... path while adapter_for returns
    a local one. Comparing paths made the assertion impossible to pass where its
    own docstring says to run it — so it compares the run NAME."""
    # Names taken from adapter_for, not written out: under SMOKE the directory
    # is `smoke_lora_r8`, and a hardcoded name would make this test assert that
    # a real generation validates against a smoke checkout — which is a genuine
    # mismatch the check SHOULD catch.
    remote = [
        {"poem_id": 1, "arm": "lora_r8", "interpretation": "x",
         "adapter": f"/content/poetry-grounding/results/adapters/"
                    f"{inference.adapter_for('lora_r8').name}"},
        {"poem_id": 2, "arm": "lora_r16", "interpretation": "x",
         "adapter": f"/kaggle/working/repo/results/adapters/"
                    f"{inference.adapter_for('lora_r16').name}"},
    ]
    inference.assert_routing(remote, {1, 2})            # must not raise


def test_a_wrong_adapter_is_still_caught_across_environments():
    """The relaxation must not swallow the failure it exists to detect."""
    bad = [{"poem_id": 1, "arm": "lora_r8", "interpretation": "x",
            "adapter": "/content/poetry-grounding/results/adapters/lora_r16"}]
    try:
        inference.assert_routing(bad, {1})
    except AssertionError as error:
        assert "lora_r16" in str(error)
        return
    raise AssertionError("an arm generated by another arm's adapter passed")
