...
def assert_routing(outputs: list[dict], test_ids: set | None = None) -> None:
    ...
    for record in outputs:
        ...
        # **Compared by NAME, not by path.** Generation runs on Kaggle or Colab
        # and this audit runs on the laptop, so the recorded string is
        # /content/.../adapters/lora_r8 while adapter_for returns the local
        # path. Comparing absolute paths made this assertion impossible to pass
        # in the place its own docstring says to run it. The directory name is
        # the run name, which is what carries the meaning — which weights
        # produced this output — and it is the same in both environments.
        recorded = PurePosixPath(record["adapter"]).name if record["adapter"] else None
        wanted = Path(expected).name if expected else None
        assert recorded == wanted, (
            f"poem {record['poem_id']} ({arm}) was generated by adapter "
            f"{recorded!r}, but {arm} generates from {wanted!r}")
